...
def predict_category(raw_text: str):
    ...
    ml_category = _category_model.classes_[best_index]
    ml_confidence = round(float(proba[best_index]), 4)
...
def predict_severity(raw_text: str):
    ...
    best_index = proba.argmax()
    ml_prediction = _severity_model.classes_[best_index]        # low / medium / high
    confidence = round(float(proba[best_index]), 4)
