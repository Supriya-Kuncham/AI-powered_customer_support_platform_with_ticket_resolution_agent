...
def init_db():
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS tickets (
            ...
            jira_ticket_key VARCHAR(30),
            jira_ticket_url VARCHAR(255),
...
def insert_ticket(employee_name, email, title, description, category,
                   ...
                   validation_confidence=None, validation_status=None,
                   jira_ticket_key=None, jira_ticket_url=None,
                   email_sent=None, workflow_log=None):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute(
        """
        INSERT INTO tickets
            (employee_name, email, title, description, department,
             ...
             resolution, retrieved_docs, rag_duration_ms,
             validation_confidence, validation_status,
             jira_ticket_key, jira_ticket_url, email_sent, workflow_log,
             status, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            ...
            json.dumps(retrieved_docs or []), rag_duration_ms,
            validation_confidence, validation_status,
            jira_ticket_key, jira_ticket_url, email_sent,
            json.dumps(workflow_log or []),
...
def _deserialize_ticket(row):
    ...
    try:
        ...
        t["retrieved_docs"] = []
    try:
        t["workflow_log"] = json.loads(t.get("workflow_log") or "[]")
    except (TypeError, json.JSONDecodeError):
        t["workflow_log"] = []
    return t


def get_all_tickets(limit=100, email=None):
    """
    Without email: returns tickets across all users (used by the REST API
    for machine-to-machine consumers). With email: scoped to just that
    user's own tickets - used by the browser UI so one account never sees
    another account's tickets or employee names.
    """
    conn = get_connection()
    cur = conn.cursor()
    if email:
        cur.execute("SELECT * FROM tickets WHERE email = ? ORDER BY ticket_id DESC LIMIT ?", (email, limit))
    else:
...
def get_stats(email=None):
    """Aggregate counts used by the dashboard charts. Pass email to scope
    to one user's own tickets; omit for an org-wide view (used by the API)."""
    conn = get_connection()
    cur = conn.cursor()
    where = "WHERE email = ?" if email else ""
    params = (email,) if email else ()

    cur.execute(f"SELECT category, COUNT(*) as c FROM tickets {where} GROUP BY category ORDER BY c DESC", params)
    by_category = [{"label": r["category"], "count": r["c"]} for r in cur.fetchall()]

    cur.execute(f"SELECT severity, COUNT(*) as c FROM tickets {where} GROUP BY severity", params)
    by_severity = [{"label": r["severity"], "count": r["c"]} for r in cur.fetchall()]

    cur.execute(f"SELECT priority, COUNT(*) as c FROM tickets {where} GROUP BY priority ORDER BY priority", params)
    by_priority = [{"label": r["priority"], "count": r["c"]} for r in cur.fetchall()]

    cur.execute(f"SELECT COUNT(*) as c FROM tickets {where}", params)
    total = cur.fetchone()["c"]

    dur_where = (where + " AND" if where else "WHERE") + " rag_duration_ms IS NOT NULL"
    cur.execute(f"SELECT AVG(rag_duration_ms) as a FROM tickets {dur_where}", params)
    avg_row = cur.fetchone()
    avg_response_time_ms = round(avg_row["a"], 0) if avg_row["a"] is not None else None

    fb_where = (where + " AND" if where else "WHERE") + " resolved IS NOT NULL"
    cur.execute(f"SELECT COUNT(*) as c FROM tickets {fb_where}", params)
    feedback_total = cur.fetchone()["c"]
    fb_yes_where = (where + " AND" if where else "WHERE") + " resolved = 'yes'"
    cur.execute(f"SELECT COUNT(*) as c FROM tickets {fb_yes_where}", params)
